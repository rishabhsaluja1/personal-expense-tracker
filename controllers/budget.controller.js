...
const setBudget = async (req, res) => {
  try {
    ...
    if (!month || !total_budget) {
      return res.status(400).json({ error: "month and total_budget are required" });
    }
    ...
    if (isNaN(Number(total_budget)) || Number(total_budget) <= 0) {
    ...
    res.json({ message: "Budget saved", budget: result.rows[0] });
  } catch (err) {
